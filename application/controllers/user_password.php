...
class User_Password extends CI_Controller {
	...
	function check_pin() {
		$result = $this->usermodel->checkPIN($this->current_user->ID, $this->input->post('pin'));
		echo $result;
	}
	...
	function password_reset(){
     ...
     //log
     $ApplicationSpecificText = "Updated Reset Password";
	...
	function changePassword(){
        ...
        //log
        $ApplicationSpecificText = "Change Password Password";
        $this->mylib->action_audit_log($ApplicationSpecificText, "SEC", "U", $this->current_user->ID, 0);
			}else{
	...
	function setOrgID(){
		...
		if($this->input->get('orgid')){
			...
			if($orgid && !empty($orgid)){
				$this->usermodel->update($this->current_user->ID, array('Org_ID' => $orgid));
        $this->session->set_userdata('ORG_ID', $orgid);
        
        //log
        $ApplicationSpecificText = "Change ORG ID";
        $this->mylib->action_audit_log($ApplicationSpecificText, "SEC", "U", $this->current_user->ID, 0);
	...
	function checkUser(){
		$response['status'] = false;
		$response['message'] = "";
		$change_org = ($this->input->get('change_org')? true:false);
		...
		if($this->current_user->User_Id){
			...
			if(!$this->session->userdata('ORG_ID') || $change_org){
