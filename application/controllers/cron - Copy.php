...
class Cron extends CI_Controller
{
  ...
  function index()
  {
    $tml1_ID = $this->current_user['tml1_ID'];; //4661;//4497 //prod
    ...
    // $sql = "SELECT  TOP 1 EncounterHistory.Encounter_ID , ReportLog.ID as log_enq_ID,  Appointments.*  FROM Wellness_eCastEMR_Data.dbo.Appointments Appointments JOIN Wellness_eCastEMR_Data.dbo.EncounterHistory EncounterHistory ON EncounterHistory.Appointments_ID=Appointments.Appointments_ID  LEFT JOIN Wellness_eCastEMR_Template.dbo.TheoConnect TheoConnect ON TheoConnect.Encounter_ID = EncounterHistory.Encounter_ID  LEFT JOIN Wellness_eCastEMR_Data.dbo.ReportLog ReportLog ON ReportLog.ID = EncounterHistory.Encounter_ID and ReportLog.ReportCategory = 'PROVIDER'  WHERE ReportLog.ID IS NULL AND Appointments.Org_ID=$org_id  ORDER BY Appointments.Appointments_ID DESC";
    ...
    foreach ($query->result() as $Appointments) {
      ...
      $this->add_template_data((int) $Encounter_ID, (int) $tml1_ID, $this->current_user['org_id']);
  ...
  function add_template_data($encounter_ID, $tml1, $org_id)
  {
    echo '<pre>encounter_id =' . $encounter_ID . '<br>';
    echo 'tml1 =' . $tml1 . '<br>';
    $encounter = $this->get_db_encounter_row($encounter_ID);
    $check_tml1_theo = $this->check_theo_link($tml1, $encounter, $this->current_user['org_id']);
    ...
    if ($tml1) {
      ...
      foreach ($template2 as $val) {
        ...
        foreach ($tml_3 as $tml3) {
          ...
          print_r($post_params);
          $this->save_tml3($post_params);
        }
      }
    }
    $this->generate_careplan($encounter->Encounter_ID, $check_theo_connect->TheoSession_ID);
    $this->generate_report($encounter->Encounter_ID, $this->current_user['org_id']);
  }
  private function get_db_encounter_row($encounterKey = null)
  {
    ...
    $dt = $this->EncounterHistoryModel->patient_history(
      ...
      $encounterKey,
      array(
        'EncounterHistory.Org_ID' => $this->current_user['org_id']
      )
    )->row();
    if ($dt) {
  ...
  private function check_theo_link($tml1, $encounter, $org_id)
  {
    echo "tml1=" . $tml1 . '<br>';
      ...
      if (!empty($tml1_dt->TML1_Theo_Link) && $tml1_dt->TML1_Theo_Link == 1) {
        ...
        if (
          ...
          if (!empty($tml1)) {
            ...
            //input session theo
            $this->inputSessionTheo($encounter, $tml1, $this->current_user['org_id']);
            $check_theo_connect  = $this->TheoConnectModel->get_where($theo_connect_data)->row();
  ...
  private function inputSessionTheo($encounter, $tml1, $org_id)
  {
    ...
    if (!empty($response['videoPlayId']) &&  !empty($response['sessionId']) && !empty($response['accountId'])) {
      ...
      $input_theo_connect = array(
        ...
        'User_ID' => $this->current_user['User_Id'], //$this->current_user->User_Id,
        'Org_ID' => $this->current_user['org_id'], // $this->current_user->org_id,
  ...
  public function generate_report($encounterKey = 0, $org_id)
  {
    ...
    if ($dt->EncounterSignedOff != 1 || $check == 0 || $check2 > 0) {
      $this->generate_all_reports($this->current_user['org_id'], $dt, 'PROVIDER', 0, 0);
      // $this->generate_all_reports($dt, 'PATIENT', 1, 0);
      // $this->generate_all_reports($dt, 'SUMMARY', 0, 1);
    }

    //xml generate
    $this->save_report('provider', $encounterKey, 'xml', 1, $this->current_user['org_id']);
  ...
  private function generate_all_reports($org_id, $dt, $cat, $PrintPatientOnly = 0, $summary_report = false, $print_mode = '')
  {
    $data = $this->data_set();
    ...
    $data_ins = $q + array(
      ...
      'Org_ID' => $this->current_user['org_id'],
      'User_PK' => $this->current_user['User_PK'],
  ...
  public function save_report($cat = NULL, $encounterKey = 0, $format = NULL, $is_donwload = 0, $org_id)
  {
    $dt = $this->get_db_encounter_row($encounterKey);
  ...
  public function save()
  {
    ...
    foreach ($query->result() as $Appointments) {
      ...
      echo '<pre>Appointments_ID =' . $Appointments_ID . '<br>';
      $this->encounter((int) $Encounter_ID, $AdvanceMDTemp_ID);
      // exit(print_r($Appointments->Appointments_ID));
    }
  }


  public function encounter($encounter_id = 0, $AdvanceMDTemp_ID, $print = NULL)
  {
    ...
    //update view report
    $data_up = array('ClinicalTriggerView' => 1,'Dept_ID' =>$this->current_user['Dept_ID']);
    ...
    $this->save_as_pdf($encounter_id, $AdvanceMDTemp_ID);
    // }
    // else { 
  ...
  private function save_as_pdf($encounter_id)
  {
    ...
    $this->send_xml($encounter_id);
  }

  public function send_xml($encounterID)
  {
...
<hwdocument>
  <ppmdmsglist>';
    ...
    // $path  = "./reports/xml/" . $this->current_user['org_id'];
    // if (!file_exists($path)) {
    //   mkdir($path, 0777, true);
    // }
    // $url_xml =  $path . '/' . '[' . SERVER_APP . ']-[' . $encounterID . ']-ppmdmsglist.xml';
    // $xml_file = file_put_contents($url_xml, $xml, LOCK_EX); 
  ...
  function signedReport()
  {
    $tml1_ID = $this->current_user['tml1_ID'];; //4661;//4497 //prod
    $org_id = $this->current_user['org_id'];
    $sql = "SELECT TOP 1 * FROM [Wellness_eCastEMR_Data].[dbo].[EncounterHistory] where UpdatedEncounter=1 and Org_ID=$org_id";
    // $sql = "SELECT  TOP 1 EncounterHistory.Encounter_ID , ReportLog.ID as log_enq_ID,  Appointments.*  FROM Wellness_eCastEMR_Data.dbo.Appointments Appointments JOIN Wellness_eCastEMR_Data.dbo.EncounterHistory EncounterHistory ON EncounterHistory.Appointments_ID=Appointments.Appointments_ID  LEFT JOIN Wellness_eCastEMR_Template.dbo.TheoConnect TheoConnect ON TheoConnect.Encounter_ID = EncounterHistory.Encounter_ID  LEFT JOIN Wellness_eCastEMR_Data.dbo.ReportLog ReportLog ON ReportLog.ID = EncounterHistory.Encounter_ID and ReportLog.ReportCategory = 'PROVIDER' LEFT JOIN Wellness_eCastEMR_Data.dbo.TabletTriggersData TabletTriggers ON TabletTriggers.Encounter_ID = EncounterHistory.Encounter_ID WHERE  EncounterHistory.UpdatedEncounter=1 AND Appointments.Org_ID=$org_id  ORDER BY Appointments.Appointments_ID DESC";
    // $sql = "SELECT  TOP 1 EncounterHistory.Encounter_ID , ReportLog.ID as log_enq_ID,  Appointments.*  FROM Wellness_eCastEMR_Data.dbo.Appointments Appointments JOIN Wellness_eCastEMR_Data.dbo.EncounterHistory EncounterHistory ON EncounterHistory.Appointments_ID=Appointments.Appointments_ID  LEFT JOIN Wellness_eCastEMR_Template.dbo.TheoConnect TheoConnect ON TheoConnect.Encounter_ID = EncounterHistory.Encounter_ID  LEFT JOIN Wellness_eCastEMR_Data.dbo.ReportLog ReportLog ON ReportLog.ID = EncounterHistory.Encounter_ID and ReportLog.ReportCategory = 'PROVIDER'  WHERE ReportLog.ID IS NULL AND Appointments.Org_ID=$org_id  ORDER BY Appointments.Appointments_ID DESC";
    ...
    foreach ($query->result() as $Appointments) {
      ...
      // echo '<pre>Appointments_ID =' . $Appointments_ID . '<br>';
      // $this->add_template_data((int) $Encounter_ID, (int) $tml1_ID, $this->current_user['org_id']);
      $this->generate_signed_report((int) $Encounter_ID, $this->current_user['org_id']);
      // exit(print_r($Appointments->Appointments_ID));
    }
    // echo '$query->result() <br>';
    // exit(print_r($query->result()));
  ...
  public function generate_signed_report($encounterKey = 0, $org_id)
  {
    $dt = $this->get_db_encounter_row($encounterKey);
...
// t1.Encounter_ID,t1.TabletTriggers_ID,t4.AccountNumber,t2.TTPhysician,t3.AdvanceMDTemp_ID from TabletTriggersData as t1 JOIN TabletTriggers AS t2 on t1.TabletTriggers_ID=t2.TabletTriggers_ID JOIN Actone_HwTemplates AS t3 on t1.TabletTriggers_ID=t3.TabletTriggers_ID JOIN PatientProfile as t4 on t1.Patient_ID=t4.Patient_ID where t1.Org_ID=341 AND t2.TabletTriggers_ID IN (666,701) and t1.Patient_ID=1000587936