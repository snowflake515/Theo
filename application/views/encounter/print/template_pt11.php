...
foreach ($CountTML1IDs_result as $CountTML1IDs_dt) {
  ...
  //         AND (TI.Status <> 'X' OR TI.Status IS NULL)
    ...
    foreach ($TML_result as $TML_dt) {
      ...
      //  $key_order_id = $TML_dt->t1s . '_' . $TML_dt->t2s . '_' . $TML_dt->t3s;
        if (trim(strtolower($TML_dt->TML3_TextToType)) != "[input]") {
          ...
          //   );
          // }
         // $join_text_tml3[] = '__[input]' . trim($TML_dt->TML3_Value);
  ...
  /*  $count_join = count($join_text_tml3);
    if ($count_join >= 2) {
	  	if (strpos($join_text_tml3[$count_join - 1], '__[input]') === FALSE){
